...
function toGlobalGuideLines(guideLines, ctx) {
  return (guideLines || []).map((guide) => ({
    ...guide,
    points: (guide.points || []).map((point) => ({
      ...point,
      index: ctx.startIndex + clamp(point.index, 0, Math.max(ctx.length - 1, 0)),
    })),
  }));
}
...
function buildBaseResult(spec, candles, ctx, metrics, intervalLabel) {
  ...
  return {
    ...
    span,
    tracePoints: (metrics.tracePoints || []).slice().sort((left, right) => left.index - right.index || left.price - right.price),
    guideLines: toGlobalGuideLines(metrics.guideLines || [], ctx),
    bias: spec.bias,
