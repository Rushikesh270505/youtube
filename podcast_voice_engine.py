...
DEFAULT_MODEL_ID = "local_voice_pack"
SPEAKER_PAUSE_MS = {
  "analyst_1": 240,
  "analyst_2": 320,
  "narrator": 380,
...
def build_timeframe_turns(entry: Dict[str, Any], coin_pair: str, previous_entry: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
  ...
  analyst_1_open = (
    ...
    f"with about {confidence} percent confidence, so right now it seems {bias}."
  )
  analyst_2_validate = (
    ...
    f"and to me that matters because {clean_reasoning(entry['reasoning'])}"
  )
  if direction == "Bullish":
...
def build_summary_turn(entries: List[Dict[str, Any]], coin_pair: str) -> List[Dict[str, Any]]:
  ...
  text = (
    ...
    f"and the breakout probability looks near {summary['breakout_probability']} percent. We still let price prove it."
  )
...
def build_roundtable_summary_turns(entries: List[Dict[str, Any]], coin_pair: str) -> List[Dict[str, Any]]:
  ...
  narrator_open = (
    f"Here is the final read on {coin_pair}... "
    f"Across the full six-timeframe stack, the market currently seems {summary['overall_trend']}. "
    f"Key support is around {summary['key_support']}, and resistance is around {summary['key_resistance']}."
  )
  analyst_1 = (
    f"My fast takeaway is that {top_pattern} on the {top_interval} is still setting the tone... "
    f"so if momentum picks up near {summary['key_resistance']}, this can still stretch quickly."
  )
  analyst_2 = (
    f"I agree with the broader read, but I still want a clean reaction first. "
    f"If price keeps respecting {summary['key_support']}, the pressure can stay balanced. "
    f"If that level slips, the tone can change quickly."
  )
  narrator_close = (
    f"So the working view is {summary['overall_trend']}, with breakout odds near {summary['breakout_probability']} percent... "
    "but we still let price prove it."
  )
  return [
...
def build_news_turns(news_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
  ...
  for index, item in enumerate(news_items[:3]):
    ...
    narrator = (
      f"{pick_phrase(['Here is one calm headline check', 'Another crypto headline worth watching', 'One more measured headline read'], f'news:{index}:intro')}"
      f"{dynamic_break(f'news:{index}:intro-break', 440, 620, 760)}"
      f"{news['source']} is reporting {emphasis(news['title'])}. "
      f"{dynamic_break(f'news:{index}:summary-break', 380, 540, 680)}"
      f"{news['summary']}"
    )
    turns.extend(
      [
...
def build_live_brief_turns(payload: Dict[str, Any], coin_pair: str) -> List[Dict[str, Any]]:
  ...
  if event_type == "indicator_observation":
  ...
  return [
    ...
    {"speaker_id": "analyst_1", "stage": "live", "text": analyst_1_follow_up, "event_type": event_type},
  ]


def resolve_cast(requested_ids: Optional[List[str]] = None) -> Dict[str, PodcastSpeaker]:
  packs_by_id = voice_service.load_voice_packs()
  selected_packs = voice_service.select_voice_packs(requested_ids)
  ...
  def assign_by_role(podcast_role: str, fallback_index: int) -> Optional[voice_service.VoicePack]:
    for pack in selected_packs:
      if pack.podcast_role == podcast_role:
        return pack
    if selected_packs and 0 <= fallback_index < len(selected_packs):
      return selected_packs[fallback_index]
    return None

  analyst_1_pack = assign_by_role("analyst_1", 1)
  analyst_2_pack = assign_by_role("analyst_2", 0)
  narrator_pack = assign_by_role("narrator", 2 if len(selected_packs) > 2 else len(selected_packs) - 1)

  cast: Dict[str, PodcastSpeaker] = {}
  for speaker_id, pack in {
    "analyst_1": analyst_1_pack,
    "analyst_2": analyst_2_pack,
    "narrator": narrator_pack,
  }.items():
    if not pack:
      continue
...
def get_podcast_voice_status() -> Dict[str, Any]:
  ...
  cast = resolve_cast(catalog.get("default_voice_pack_ids"))
  speakers = []
  for speaker_id in ("analyst_1", "analyst_2", "narrator"):
    ...
    if not member:
      ...
      continue
    runtime = voice_service.resolve_voice_runtime(voice_service.load_voice_packs()[member.voice_pack_id])
    speakers.append(
      {
        "id": speaker_id,
        "name": member.name,
        ...
        "active_voice_label": voice_service.active_voice_label(voice_service.load_voice_packs()[member.voice_pack_id], runtime),
      }
    )
  ...
  return {
    ...
    "default_voice_pack_ids": catalog.get("default_voice_pack_ids", voice_service.DEFAULT_VOICE_PACK_IDS),
...
def render_podcast_response(payload: Dict[str, Any]) -> Dict[str, Any]:
  ...
  selected_packs = voice_service.select_voice_packs(payload.get("voice_pack_ids"))
  runtimes = {pack.id: voice_service.resolve_voice_runtime(pack) for pack in selected_packs}
  ...
  for index, turn in enumerate(script_package["turns"]):
    ...
    audio_path = voice_service.ensure_clip_audio(pack, runtime, turn["plain_text"], session_dir, clip_id)
